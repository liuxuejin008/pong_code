...
CREATE TABLE IF NOT EXISTS `sprint` (
  ...
  `code_prefix` VARCHAR(3) NULL,
  `next_item_number` INT NULL,
  `project_id` INT NULL,
  `owner_id` INT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_sprint_code_prefix` (`code_prefix`),
...
CREATE TABLE IF NOT EXISTS `bug` (
  ...
  `bug_type` VARCHAR(32) NOT NULL DEFAULT 'functional',
  `priority` VARCHAR(16) NOT NULL DEFAULT 'normal',
  `platform` VARCHAR(32) NOT NULL DEFAULT 'server',
  `discovery_phase` VARCHAR(32) NOT NULL DEFAULT 'round_1',
  `discovery_channel` VARCHAR(32) NULL DEFAULT NULL,
